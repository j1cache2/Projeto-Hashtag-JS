import{l as x,c as a,s as c}from"./utilidades-5b4575e0.js";const o=x("carrinho")??{};function y(){document.getElementById("carrinho").classList.add("right-[0px]"),document.getElementById("carrinho").classList.remove("right-[-360px]")}function C(){document.getElementById("carrinho").classList.remove("right-[0px]"),document.getElementById("carrinho").classList.add("right-[-360px]")}function b(){Object.keys(o).length!==0&&(window.location.href="./checkout.html")}function v(){const t=document.getElementById("fechar-carrinho"),e=document.getElementById("abrir-carrinho"),n=document.getElementById("finalizar-compra");t.addEventListener("click",C),e.addEventListener("click",y),n.addEventListener("click",b)}function m(t){delete o[t],c("carrinho",o),i(),g()}function u(t){o[t]++,c("carrinho",o),i(),f(t)}function $(t){if(o[t]===1){m(t);return}o[t]--,c("carrinho",o),i(),f(t)}function f(t){document.getElementById(`quantidade-${t}`).innerText=o[t]}function h(t){const e=a.find(d=>d.id===t),n=document.getElementById("produtos-carrinho"),r=document.createElement("article"),p=["flex","bg-slate-100","rounded-lg","p-1","relative"];for(const d of p)r.classList.add(d);const E=`<button id="remover-item-${e.id}"><i class="fa-solid fa-circle-xmark text-slate-500 absolute top-0 right-2 hover:text-slate-800"></i></button>
    <img src="./assets/img/${e.imagem}" alt="Carrinho: ${e.nome}" class="h-24 rounded-lg">
    ...
    <button id="adicionar-${t.id}" class="bg-slate-950 text-slate-200 hover:bg-slate-700"><i class="fa-solid fa-cart-plus"></i></button>
